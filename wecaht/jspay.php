...
class jspay
{
     ...
     * 微信退款的订单
     */
    public function refund($op_user_id, $out_refund_no, $out_trade_no, $refund_fee, $total_fee)
    {
        $refund = array(
            'appid' => $this->appid,
            'mch_id' => $this->mchid,
        ...
        $refund['sign'] =self::getSign($refund, $this->key);
        ...
        if ($responseXml) {
            $data = simplexml_load_string($responseXml, 'SimpleXMLElement', LIBXML_NOCDATA);
            curl_close($ch);
            var_dump($data);
        } else {
            $error=curl_errno($ch);
    ...
    public function notify()
    {
        ...
        file_put_contents('notify.xml', $postStr);
